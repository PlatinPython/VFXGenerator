...
public class VFXGeneratorTileEntity extends TileEntity implements ITickableTileEntity {
    ...
    private TreeSet<ResourceLocation> particlesSelected = Util.createTreeSetFromCollectionWithComparator(Arrays.asList(
            new ResourceLocation(VFXGenerator.MOD_ID, "particle/spark_small"),
            new ResourceLocation(VFXGenerator.MOD_ID, "particle/spark_mid"),
            new ResourceLocation(VFXGenerator.MOD_ID, "particle/spark_big")), ResourceLocation::compareNamespaced);
        ...
        if ((this.getBlockState().getValue(VFXGeneratorBlock.INVERTED) &&
            ...
            if (world.isClientSide && this.particleEnabled) {
                if (world.getGameTime() % particleDelay == 0) {
                    ...
                    ResourceLocation particle = new ArrayList<>(particlesSelected)
                            .get(random.nextInt(particlesSelected.size()));
                    ...
                    int lifetime = Math.round((particleLifetimeBot +
                                               (random.nextFloat() * (particleLifetimeTop - particleLifetimeBot))));
                    ...
                    double spawnX = pos.x +
                                    particleSpawnXBot +
                                    (random.nextFloat() * (particleSpawnXTop - particleSpawnXBot));
                    double spawnY = pos.y +
                                    particleSpawnYBot +
                                    (random.nextFloat() * (particleSpawnYTop - particleSpawnYBot));
                    double spawnZ = pos.z +
                                    particleSpawnZBot +
                                    (random.nextFloat() * (particleSpawnZTop - particleSpawnZBot));
                    pos = new Vector3d(spawnX, spawnY, spawnZ);

                    double motionX = particleMotionXBot +
                                     (random.nextFloat() * (particleMotionXTop - particleMotionXBot));
                    double motionY = particleMotionYBot +
                                     (random.nextFloat() * (particleMotionYTop - particleMotionYBot));
                    double motionZ = particleMotionZBot +
                                     (random.nextFloat() * (particleMotionZTop - particleMotionZBot));
    ...
    private CompoundNBT saveToParticleTag() {
        ...
        getParticlesSelected().forEach((location) -> listNBT.add(StringNBT.valueOf(location.toString())));
    ...
    private void loadFromParticleTag(CompoundNBT particleTag) {
        ...
        if (particleTag.getTagType("selected") == Constants.NBT.TAG_LIST) {
            particlesSelected = Util.createTreeSetFromCollectionWithComparator(((ListNBT) particleTag.get("selected")).stream()
                                                                                                                      .map((nbt) -> ResourceLocation.tryParse(
                                                                                                                              nbt.getAsString()))
                                                                                                                      .filter(ParticleConstants.PARTICLE_OPTIONS::contains)
                                                                                                                      .collect(
                                                                                                                              Collectors.toList()),
                                                                               ResourceLocation::compareNamespaced);
            if (particlesSelected.isEmpty())
                particlesSelected = Util.createTreeSetFromCollectionWithComparator(Collections.singletonList(new ResourceLocation(
                        VFXGenerator.MOD_ID,
                        "particle/circle")), ResourceLocation::compareNamespaced);
            if (particlesSelected.size() > ParticleConstants.PARTICLE_OPTIONS.size())
                particlesSelected = Util.createTreeSetFromCollectionWithComparator(ParticleConstants.PARTICLE_OPTIONS,
                                                                                   ResourceLocation::compareNamespaced);
        } else {
            particlesSelected = Util.createTreeSetFromCollectionWithComparator(Collections.singletonList(
                                                                                       ParticleConstants.PARTICLE_OPTIONS.contains(
                                                                                               new ResourceLocation(VFXGenerator.MOD_ID, "particle/" + particleTag.getString("selected")))
                                                                                       ? new ResourceLocation(
                                                                                               VFXGenerator.MOD_ID,
                                                                                               "particle/" + particleTag.getString("selected"))
                                                                                       : new ResourceLocation(
                                                                                               VFXGenerator.MOD_ID,
                                                                                               "particle/circle")),
                                                                               ResourceLocation::compareNamespaced);
        ...
        particleLifetimeBot = MathHelper.clamp(particleTag.getInt("lifetimeBot"),
                                               ParticleConstants.MIN_LIFETIME,
                                               ParticleConstants.MAX_LIFETIME);
        particleLifetimeTop = MathHelper.clamp(particleTag.getInt("lifetimeTop"),
                                               ParticleConstants.MIN_LIFETIME,
                                               ParticleConstants.MAX_LIFETIME);
        particleSizeBot = MathHelper.clamp(particleTag.getFloat("sizeBot"),
                                           ParticleConstants.MIN_SIZE,
                                           ParticleConstants.MAX_SIZE);
        particleSizeTop = MathHelper.clamp(particleTag.getFloat("sizeTop"),
                                           ParticleConstants.MIN_SIZE,
                                           ParticleConstants.MAX_SIZE);
        particleSpawnXBot = MathHelper.clamp(particleTag.getFloat("spawnXBot"),
                                             ParticleConstants.MIN_SPAWN,
                                             ParticleConstants.MAX_SPAWN);
        particleSpawnXTop = MathHelper.clamp(particleTag.getFloat("spawnXTop"),
                                             ParticleConstants.MIN_SPAWN,
                                             ParticleConstants.MAX_SPAWN);
        particleSpawnYBot = MathHelper.clamp(particleTag.getFloat("spawnYBot"),
                                             ParticleConstants.MIN_SPAWN,
                                             ParticleConstants.MAX_SPAWN);
        particleSpawnYTop = MathHelper.clamp(particleTag.getFloat("spawnYTop"),
                                             ParticleConstants.MIN_SPAWN,
                                             ParticleConstants.MAX_SPAWN);
        particleSpawnZBot = MathHelper.clamp(particleTag.getFloat("spawnZBot"),
                                             ParticleConstants.MIN_SPAWN,
                                             ParticleConstants.MAX_SPAWN);
        particleSpawnZTop = MathHelper.clamp(particleTag.getFloat("spawnZTop"),
                                             ParticleConstants.MIN_SPAWN,
                                             ParticleConstants.MAX_SPAWN);
        particleMotionXBot = MathHelper.clamp(particleTag.getFloat("motionXBot"),
                                              ParticleConstants.MIN_MOTION,
                                              ParticleConstants.MAX_MOTION);
        particleMotionXTop = MathHelper.clamp(particleTag.getFloat("motionXTop"),
                                              ParticleConstants.MIN_MOTION,
                                              ParticleConstants.MAX_MOTION);
        particleMotionYBot = MathHelper.clamp(particleTag.getFloat("motionYBot"),
                                              ParticleConstants.MIN_MOTION,
                                              ParticleConstants.MAX_MOTION);
        particleMotionYTop = MathHelper.clamp(particleTag.getFloat("motionYTop"),
                                              ParticleConstants.MIN_MOTION,
                                              ParticleConstants.MAX_MOTION);
        particleMotionZBot = MathHelper.clamp(particleTag.getFloat("motionZBot"),
                                              ParticleConstants.MIN_MOTION,
                                              ParticleConstants.MAX_MOTION);
        particleMotionZTop = MathHelper.clamp(particleTag.getFloat("motionZTop"),
                                              ParticleConstants.MIN_MOTION,
                                              ParticleConstants.MAX_MOTION);
        particleDelay = MathHelper.clamp(particleTag.getInt("delay"),
                                         ParticleConstants.MIN_DELAY,
                                         ParticleConstants.MAX_DELAY);
        particleGravity = MathHelper.clamp(particleTag.getFloat("gravity"),
                                           ParticleConstants.MIN_GRAVITY,
                                           ParticleConstants.MAX_GRAVITY);
    ...
    private void ensureParticleDataOrder() {
        ...
        particleLifetimeBot = MathHelper.clamp(getParticleLifetimeBot(),
                                               ParticleConstants.MIN_LIFETIME,
                                               getParticleLifetimeTop());
        particleLifetimeTop = MathHelper.clamp(getParticleLifetimeTop(),
                                               getParticleLifetimeBot(),
                                               ParticleConstants.MAX_LIFETIME);
        ...
        particleSpawnXBot = MathHelper.clamp(getParticleSpawnXBot(),
                                             ParticleConstants.MIN_SPAWN,
                                             getParticleSpawnXTop());
        particleSpawnXTop = MathHelper.clamp(getParticleSpawnXTop(),
                                             getParticleSpawnXBot(),
                                             ParticleConstants.MAX_SPAWN);
        particleSpawnYBot = MathHelper.clamp(getParticleSpawnYBot(),
                                             ParticleConstants.MIN_SPAWN,
                                             getParticleSpawnYTop());
        particleSpawnYTop = MathHelper.clamp(getParticleSpawnYTop(),
                                             getParticleSpawnYBot(),
                                             ParticleConstants.MAX_SPAWN);
        particleSpawnZBot = MathHelper.clamp(getParticleSpawnZBot(),
                                             ParticleConstants.MIN_SPAWN,
                                             getParticleSpawnZTop());
        particleSpawnZTop = MathHelper.clamp(getParticleSpawnZTop(),
                                             getParticleSpawnZBot(),
                                             ParticleConstants.MAX_SPAWN);
        particleMotionXBot = MathHelper.clamp(getParticleMotionXBot(),
                                              ParticleConstants.MIN_MOTION,
                                              getParticleMotionXTop());
        particleMotionXTop = MathHelper.clamp(getParticleMotionXTop(),
                                              getParticleMotionXBot(),
                                              ParticleConstants.MAX_MOTION);
        particleMotionYBot = MathHelper.clamp(getParticleMotionYBot(),
                                              ParticleConstants.MIN_MOTION,
                                              getParticleMotionYTop());
        particleMotionYTop = MathHelper.clamp(getParticleMotionYTop(),
                                              getParticleMotionYBot(),
                                              ParticleConstants.MAX_MOTION);
        particleMotionZBot = MathHelper.clamp(getParticleMotionZBot(),
                                              ParticleConstants.MIN_MOTION,
                                              getParticleMotionZTop());
        particleMotionZTop = MathHelper.clamp(getParticleMotionZTop(),
                                              getParticleMotionZBot(),
                                              ParticleConstants.MAX_MOTION);
    }
    ...
    public void setParticlesSelected(TreeSet<ResourceLocation> particlesSelected) {
        this.particlesSelected = Util.createTreeSetFromCollectionWithComparator(particlesSelected.stream()
                                                                                                 .filter(ParticleConstants.PARTICLE_OPTIONS::contains)
                                                                                                 .collect(Collectors.toList()),
                                                                                ResourceLocation::compareNamespaced);
        if (this.particlesSelected.isEmpty())
            this.particlesSelected = Util.createTreeSetFromCollectionWithComparator(Collections.singletonList(new ResourceLocation(
                    VFXGenerator.MOD_ID,
                    "particle/circle")), ResourceLocation::compareNamespaced);
        setChanged();
    }
    ...
    public void setParticleLifetimeBot(int particleLifetimeBot) {
        this.particleLifetimeBot = MathHelper.clamp(particleLifetimeBot,
                                                    ParticleConstants.MIN_LIFETIME,
                                                    getParticleLifetimeTop());
        setChanged();
    }
    ...
    public void setParticleLifetimeTop(int particleLifetimeTop) {
        this.particleLifetimeTop = MathHelper.clamp(particleLifetimeTop,
                                                    getParticleLifetimeBot(),
                                                    ParticleConstants.MAX_LIFETIME);
        setChanged();
    }
    ...
    public void setParticleSpawnXBot(float particleSpawnXBot) {
        this.particleSpawnXBot = MathHelper.clamp(particleSpawnXBot,
                                                  ParticleConstants.MIN_SPAWN,
                                                  getParticleSpawnXTop());
        setChanged();
    }
    ...
    public void setParticleSpawnXTop(float particleSpawnXTop) {
        this.particleSpawnXTop = MathHelper.clamp(particleSpawnXTop,
                                                  getParticleSpawnXBot(),
                                                  ParticleConstants.MAX_SPAWN);
        setChanged();
    }
    ...
    public void setParticleSpawnYBot(float particleSpawnYBot) {
        this.particleSpawnYBot = MathHelper.clamp(particleSpawnYBot,
                                                  ParticleConstants.MIN_SPAWN,
                                                  getParticleSpawnYTop());
        setChanged();
    }
    ...
    public void setParticleSpawnYTop(float particleSpawnYTop) {
        this.particleSpawnYTop = MathHelper.clamp(particleSpawnYTop,
                                                  getParticleSpawnYBot(),
                                                  ParticleConstants.MAX_SPAWN);
        setChanged();
    }
    ...
    public void setParticleSpawnZBot(float particleSpawnZBot) {
        this.particleSpawnZBot = MathHelper.clamp(particleSpawnZBot,
                                                  ParticleConstants.MIN_SPAWN,
                                                  getParticleSpawnZTop());
        setChanged();
    }
    ...
    public void setParticleSpawnZTop(float particleSpawnZTop) {
        this.particleSpawnZTop = MathHelper.clamp(particleSpawnZTop,
                                                  getParticleSpawnZBot(),
                                                  ParticleConstants.MAX_SPAWN);
        setChanged();
    }
    ...
    public void setParticleMotionXBot(float particleMotionXBot) {
        this.particleMotionXBot = MathHelper.clamp(particleMotionXBot,
                                                   ParticleConstants.MIN_MOTION,
                                                   getParticleMotionXTop());
        setChanged();
    }
    ...
    public void setParticleMotionXTop(float particleMotionXTop) {
        this.particleMotionXTop = MathHelper.clamp(particleMotionXTop,
                                                   getParticleMotionXBot(),
                                                   ParticleConstants.MAX_MOTION);
        setChanged();
    }
    ...
    public void setParticleMotionYBot(float particleMotionYBot) {
        this.particleMotionYBot = MathHelper.clamp(particleMotionYBot,
                                                   ParticleConstants.MIN_MOTION,
                                                   getParticleMotionYTop());
        setChanged();
    }
    ...
    public void setParticleMotionYTop(float particleMotionYTop) {
        this.particleMotionYTop = MathHelper.clamp(particleMotionYTop,
                                                   getParticleMotionYBot(),
                                                   ParticleConstants.MAX_MOTION);
        setChanged();
    }
    ...
    public void setParticleMotionZBot(float particleMotionZBot) {
        this.particleMotionZBot = MathHelper.clamp(particleMotionZBot,
                                                   ParticleConstants.MIN_MOTION,
                                                   getParticleMotionZTop());
        setChanged();
    }
    ...
    public void setParticleMotionZTop(float particleMotionZTop) {
        this.particleMotionZTop = MathHelper.clamp(particleMotionZTop,
                                                   getParticleMotionZBot(),
                                                   ParticleConstants.MAX_MOTION);
        setChanged();
    }
    ...
    public void setParticleGravity(float particleGravity) {
        this.particleGravity = MathHelper.clamp(particleGravity,
                                                ParticleConstants.MIN_GRAVITY,
                                                ParticleConstants.MAX_GRAVITY);
        setChanged();
    }
