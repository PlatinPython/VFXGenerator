...
public class FloatRangeSlider extends UpdateableWidget {
    ...
    public boolean isLeftHovered(int mouseX) {
        return this.isHovered() &&
               mouseX < (this.x + ((this.rightSliderValue + this.leftSliderValue) / 2) * this.width);
    }
    ...
    protected void renderBg(MatrixStack matrixStack, Minecraft minecraft, int mouseX, int mouseY) {
        ...
        if (isLeftHovered(mouseX)) {
            this.blit(matrixStack,
                      this.x + (int) (this.rightSliderValue * (double) (this.width - 8)),
                      this.y,
                      0,
                      46 + this.getYImageNoDisabled(isRightHovered(mouseX)) * 20,
                      4,
                      this.height);
            this.blit(matrixStack,
                      this.x + (int) (this.rightSliderValue * (double) (this.width - 8)) + 4,
                      this.y,
                      196,
                      46 + this.getYImageNoDisabled(isRightHovered(mouseX)) * 20,
                      4,
                      this.height);
            this.blit(matrixStack,
                      this.x + (int) (this.leftSliderValue * (double) (this.width - 8)),
                      this.y,
                      0,
                      46 + this.getYImageNoDisabled(isLeftHovered(mouseX)) * 20,
                      4,
                      this.height);
            this.blit(matrixStack,
                      this.x + (int) (this.leftSliderValue * (double) (this.width - 8)) + 4,
    ...
    private boolean getIsLeftClicked(double mouseX) {
        return (mouseX < this.x + (int) (this.leftSliderValue * (double) (this.width - 8)) + 8 ||
                mouseX <
                ((this.x + this.leftSliderValue * this.width) + (this.x + this.rightSliderValue * this.width)) / 2);
    }
        ...
        if (flag || keyCode == GLFW.GLFW_KEY_RIGHT) {
            if (flag) {
                if (this.leftSliderValue != 0D) {
                    this.setLeftSliderValue(Util.clamp((this.getLeftSliderValue() - this.stepSize),
                                                       this.minValue,
                                                       this.maxValue,
                                                       this.stepSize));
                    this.setRightSliderValue(Util.clamp((this.getRightSliderValue() - this.stepSize),
                                                        this.minValue,
                                                        this.maxValue,
                                                        this.stepSize));
                }
    ...
    public void updateValue() {
        if (this.leftValueSupplier.get() != this.getLeftSliderValue()) {
            this.leftSliderValue = Util.clamp(this.leftValueSupplier.get(),
                                              this.minValue,
                                              this.maxValue,
                                              this.stepSize);
        }
        if (this.rightValueSupplier.get() != this.getRightSliderValue()) {
            this.rightSliderValue = Util.clamp(this.rightValueSupplier.get(),
                                               this.minValue,
                                               this.maxValue,
                                               this.stepSize);
        }
    ...
    private void setLeftSliderValue(double value) {
        ...
        this.leftSliderValue = Util.toValue(MathHelper.clamp(value, 0.0D, this.rightSliderValue),
                                            this.minValue,
                                            this.maxValue,
                                            this.stepSize);
    ...
    private void setRightSliderValue(double value) {
        ...
        this.rightSliderValue = Util.toValue(MathHelper.clamp(value, this.leftSliderValue, 1.0D),
                                             this.minValue,
                                             this.maxValue,
                                             this.stepSize);
