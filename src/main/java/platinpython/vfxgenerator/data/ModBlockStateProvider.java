...
public class ModBlockStateProvider extends BlockStateProvider {
    ...
    protected void registerStatesAndModels() {
        ...
        itemModels().withExistingParent(BlockRegistry.VFX_GENERATOR.getId().getPath(),
                                        modLoc(ModelProvider.BLOCK_FOLDER + "/vfx_generator_off")
                    )
                    .override()
    ...
    private ModelFile getVFXGeneratorModelForState(BlockState state) {
        return models().getBuilder(
                               modLoc(ModelProvider.BLOCK_FOLDER + "/vfx_generator_" + (state.getValue(VFXGeneratorBlock.POWERED) ?
                                                                                        "on" :
                                                                                        "off") + (state.getValue(
                                       VFXGeneratorBlock.INVERTED) ? "_inverted" : "")).toString())
                       .parent(models().getExistingFile(
                               modLoc(ModelProvider.BLOCK_FOLDER + "/vfx_generator_base" + (state.getValue(
                                       VFXGeneratorBlock.POWERED) ? "_on" : ""))))
                       .texture("frame", mcLoc("block/smooth_stone"))
                       .texture("glass", modLoc("block/vfx_generator_glass"))
                       .texture("redstone", modLoc("block/redstone_rod_" + (state.getValue(VFXGeneratorBlock.POWERED) ?
                                                                            "on" :
                                                                            "off")))
                       .texture("rod", modLoc("block/vfx_generator_rod" + (state.getValue(VFXGeneratorBlock.INVERTED) ?
                                                                           "_inverted" :
                                                                           "")));
    }
}
